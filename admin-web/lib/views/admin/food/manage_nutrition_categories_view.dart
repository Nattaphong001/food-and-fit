...
class _ManageNutritionCategoriesViewState extends State<ManageNutritionCategoriesView> {
  ...
  void initState() {
    ...
    _fetchCategories();
    _fetchFoodCounts();
    AdminDataBus.foodItems.addListener(_onFoodItemsChanged);
  }

  @override
  void dispose() {
    AdminDataBus.foodItems.removeListener(_onFoodItemsChanged);
    super.dispose();
  }

  // หน้านี้ถูก cache ค้างไว้ใน IndexedStack ของ admin_shell_view ไม่ได้ rebuild ตอนสลับเมนู —
  // ฟัง AdminDataBus แทน เพื่อรู้ตัวทันทีเวลาหน้าโภชนาการ (คนละหน้า คนละ state) เพิ่ม/แก้/ลบสำเร็จ
  // ไม่งั้น badge "N รายการ" ต่อการ์ดจะค้างเลขเก่าจนกว่าจะรีโหลดทั้งหน้า
                ...
                children: [
                  Container(
                    ...
                    child: Row(children: [
                      ...
                      Expanded(
                        child: Text(item == null ? 'เพิ่มประเภทโภชนาการ' : 'แก้ไขประเภทโภชนาการ',
