...
// (2026-09-06) GetAdminWeightExercises คืน `muscle_details: [{exm_type, mug_id, muscle_group:{...}},
// ...]` แทน `muscle_group: {...}` เดี่ยวๆ แบบเดิม (weight_exercises.mug_id ถูก DROP ออกจาก DB ไปแล้ว
// ตั้งแต่ 2026-09-04 — ท่าฝึกมีกล้ามเนื้อหลักได้มากกว่า 1 มัด) โค้ดหน้านี้เคยอ่าน ex['muscle_group']
// เดี่ยวๆ ตรงๆ ซึ่งไม่มีค่าอีกต่อไป (เงียบ ไม่ error แค่ได้ null เสมอ) เลยเปลี่ยนมาอ่านผ่าน list นี้แทน
// ตาม pattern เดียวกับ mobile/lib/models/exercise_model.dart
List<int> _muscleGroupIds(dynamic ex) {
  final details = (ex is Map ? ex['muscle_details'] : null) as List?;
  if (details == null) return const [];
  final ids = <int>[];
  for (final raw in details) {
    final mugId = (raw is Map ? raw['mug_id'] : null) as int?;
    if (mugId != null && !ids.contains(mugId)) ids.add(mugId);
  }
  return ids;
}

// ชื่อกล้ามเนื้อ "หลัก" ตัวแรก (exm_type=1) ไว้โชว์เป็น label ใต้ชื่อท่า — ไม่มีตัวหลัก (exm_type=1)
// เลยก็ fallback เป็นตัวแรกสุดในลิสต์กันโชว์ว่างเปล่าไปเฉยๆ
String _primaryMuscleGroupName(dynamic ex) {
  final details = (ex is Map ? ex['muscle_details'] : null) as List?;
  if (details == null || details.isEmpty) return '';
  Map? primary;
  for (final raw in details) {
    if (raw is Map && raw['exm_type'] == 1) {
      primary = raw['muscle_group'] as Map?;
      break;
    }
  }
  primary ??= (details.first is Map ? details.first['muscle_group'] as Map? : null);
  return primary?['mug_name']?.toString() ?? '';
}
...
// TickerProviderStateMixin (ไม่ใช่ Single...) — หน้านี้สร้าง TabController ใหม่ตอนแก้จำนวนวัน
// (ดู _savePlanInfo) ตัวเก่ายังไม่ทันถูก dispose ตอนสร้างตัวใหม่ (ตั้งใจเลื่อน dispose ไปเฟรมถัดไป
// กันปัญหา widget เก่ายัง removeListener จาก controller ที่ dispose ไปแล้ว) จึงมี ticker 2 ตัวมีชีวิต
// พร้อมกันชั่วขณะ — SingleTickerProviderStateMixin อนุญาตแค่ ticker เดียวตลอดอายุ state เท่านั้น
class _ManageSystemPlanDetailsViewState extends State<ManageSystemPlanDetailsView> with TickerProviderStateMixin {
                ...
                children: [
                  Container(
                    ...
                    child: Row(children: [
                      ...
                      const Expanded(child: Text('แก้ไขแผนแนะนำ', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.black87))),
  ...
  Future<void> _savePlanInfo(String name, int daysPerWeek, int difficulty, String description, File? imageFile, Uint8List? imageBytes, String? imageFileName) async {
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        // ห้าม dispose() controller เก่าใน setState นี้ตรงๆ — ตอนนั้น TabBarView/AnimatedBuilder
        // ของเฟรมก่อนหน้ายังผูกอยู่กับตัวเก่า พอ rebuild แล้วมันไป removeListener() ต่อจาก
        // controller ที่ถูก dispose ไปแล้ว Flutter จะ throw "TabController was used after being
        // disposed" ทันที (ปุ่มลด/เพิ่มจำนวนวันในหน้านี้พังเพราะจุดนี้) — เก็บตัวเก่าไว้ก่อน
        // แล้วค่อย dispose หลังเฟรมถัดไป (widget เปลี่ยนไปผูกกับตัวใหม่เรียบร้อยแล้ว)
        final oldTabController = daysChanged ? _tabController : null;
        setState(() {
          _planName = name;
        ...
        if (oldTabController != null) {
          WidgetsBinding.instance.addPostFrameCallback((_) => oldTabController.dispose());
        }
  ...
  void _showForm({Map<String, dynamic>? item}) {
    ...
    for (final ex in weightExercises) {
      final details = (ex['muscle_details'] as List?) ?? const [];
      for (final raw in details) {
        final mg = raw is Map ? raw['muscle_group'] : null;
    ...
    List<dynamic> filteredExercises() {
      ...
      return weightExercises.where((ex) {
        ...
        if (exerciseMuscleFilter != null) {
          if (!_muscleGroupIds(ex).contains(exerciseMuscleFilter)) return false;
        }
              ...
              children: [
                Container(
                  ...
                  child: Row(children: [
                    Container(width: 4, height: 20, decoration: BoxDecoration(color: AppColors.primaryGreen, borderRadius: BorderRadius.circular(2))),
                    const SizedBox(width: 10),
                    Expanded(child: Text(item == null ? 'เพิ่มท่าฝึก (เวทเทรนนิ่ง)' : 'แก้ไขท่าฝึก (เวทเทรนนิ่ง)', style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.black87))),
                          ...
                          child: Builder(builder: (context) {
                            ...
                            return ListView.builder(
                              ...
                              itemBuilder: (context, i) {
                                ...
                                final mgName = _primaryMuscleGroupName(ex);
                                return InkWell(
  ...
  Widget build(BuildContext context) {
    // header ใช้ AdminPageHeader (breadcrumb เดียว ไม่มีปุ่ม back ซ้อน — กลับไปหน้าแผนแนะนำผ่าน
    // เมนู sidebar) แบบเดียวกับหน้า drill-down อื่นในระบบ
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: Column(
          ...
          children: [
            AdminPageHeader(leading: AppBackButton(onTap: widget.onBack), breadcrumb: ['แผนแนะนำ', 'รายละเอียดแผนแนะนำ', _planName]),
            Expanded(
              child: isLoading
