...
class _ManageMuscleGroupViewState extends State<ManageMuscleGroupView> {
  ...
  void initState() {
    ...
    _fetchMuscleGroups();
    _fetchLinkedCounts();
    AdminDataBus.weightExercises.addListener(_onWeightExercisesChanged);
  }

  @override
  void dispose() {
    AdminDataBus.weightExercises.removeListener(_onWeightExercisesChanged);
    super.dispose();
  }

  // หน้านี้ถูก cache ค้างไว้ใน IndexedStack ของ admin_shell_view ไม่ได้ rebuild ตอนสลับเมนู —
  // ฟัง AdminDataBus แทน เพื่อรู้ตัวทันทีเวลาหน้าท่าฝึกเวท (คนละหน้า คนละ state) เพิ่ม/ลบท่าฝึก
  // หรือแก้กล้ามเนื้อโฟกัสสำเร็จ ไม่งั้น badge "N ท่าฝึก" ต่อการ์ดจะค้างเลขเก่าจนกว่าจะรีโหลดทั้งหน้า
  void _onWeightExercisesChanged() {
    if (!mounted) return;
    _fetchLinkedCounts();
  }
