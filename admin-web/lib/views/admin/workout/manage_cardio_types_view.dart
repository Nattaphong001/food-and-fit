...
class _ManageCardioTypesViewState extends State<ManageCardioTypesView> {
  ...
  void initState() {
    ...
    AdminDataBus.cardioActivities.addListener(_onActivitiesChanged);
  }

  @override
  void dispose() {
    AdminDataBus.cardioActivities.removeListener(_onActivitiesChanged);
    super.dispose();
  }

  // หน้านี้ถูก cache ค้างไว้ใน IndexedStack ของ admin_shell_view ไม่ได้ rebuild ตอนสลับเมนู —
  // ฟัง AdminDataBus แทน เพื่อรู้ตัวทันทีเวลาหน้ากิจกรรมคาร์ดิโอ (คนละหน้า คนละ state) เพิ่ม/แก้/ลบ
  // สำเร็จ ไม่งั้น badge "N กิจกรรม" ต่อการ์ดจะค้างเลขเก่าจนกว่าจะรีโหลดทั้งหน้า
