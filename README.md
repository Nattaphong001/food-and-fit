...
> **EN:** Food & Fit — a fitness and nutrition tracking app that calculates BMR/TDEE, logs meals and workouts, and reports daily energy balance against user goals. A 3-person thesis team project — I built the full stack: Flutter mobile app, Flutter Web admin panel, and a Go/Gin REST API on MySQL.
...
| คำนวณ BMR/TDEE (Onboarding) | สรุปพลังงานรายวัน | แผนออกกำลังกายรายสัปดาห์ |
|---|---|---|
...
|---|---|
| <img src="docs/screenshots/admin-dashboard.png" width="360"> | <img src="docs/screenshots/admin-report.png" width="360"> |
...
- เลือกแผนออกกำลังกายสำเร็จรูป (2–6 วัน/สัปดาห์) หรือสร้างแผนส่วนตัวของตัวเอง 1 แผนต่อสมาชิก แก้ไขได้อิสระ
...
**ต่อยอด — Smart Auto Calorie (เวทเทรนนิ่ง):** เดิมให้ผู้ใช้เลือกความหนัก 3 ระดับเอง (เบา/กลาง/หนัก) เพื่อกำหนด METs แต่ตรวจสอบแล้วพบว่าฟีเจอร์นี้ไม่เคยทำงานจริง — แอปมือถือไม่เคยส่งค่าที่เลือกขึ้น API เลย เปลี่ยนมาให้ระบบอนุมาน METs อัตโนมัติแทน โดยไม่ต้องพึ่งสายรัดวัดหัวใจ ผ่าน 3 เวอร์ชัน: **v1 (2026-09-08)** `Final MET = Session Base MET × k_load × k_density` โดย Session Base MET มาจาก MET พื้นฐานต่อท่าที่แอดมินตั้งไว้เอง (`wet_base_met`) — ไม่มีรหัสอ้างอิงงานวิจัยตรวจสอบย้อนกลับได้ **v2 (2026-09-17)** ลองเปลี่ยนไปเลือก MET จากรหัสกิจกรรมใน 2024 Adult Compendium of Physical Activities (Herrmann et al., 2024) ตรงๆ ต่อเซตแทน (จัดหมวดท่าฝึกก่อนแล้วพิจารณา %1RM/ความหนาแน่น) **v3 (2026-09-18, ปัจจุบัน)** ย้อนกลับมาใช้โครงสร้าง `k_load × k_density` ตามสเปกฉบับล่าสุดของเล่ม แต่เปลี่ยน Session Base MET ให้มาจาก **Reps + เวลาพักต่อเซต** แทน `wet_base_met` — บางเงื่อนไข (Reps > 15 ครั้ง) ไม่มี entry ตรงใน Compendium ต้องใช้ค่าใกล้เคียงที่สุดแทนแล้วระบุไว้ชัดเจนว่าเป็นการตีความขยาย ไม่ใช่ค่าที่ Compendium ระบุตรงๆ ทั้ง k_load (%1RM เทียบสถิติที่ดีที่สุดของสมาชิก) และ k_density (ความหนาแน่นของการฝึก) คำนวณเหมือนเดิมทุกเวอร์ชัน แล้วเข้าสูตร net METs เดิมด้านบนตรงๆ
...
**บทบาทของผม (Nattaphong) — พัฒนาระบบทั้งหมดร่วมกับทีม**
