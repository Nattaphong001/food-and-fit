...
**ต่อยอด — Smart Auto Calorie (เวทเทรนนิ่ง):** เดิมให้ผู้ใช้เลือกความหนัก 3 ระดับเอง (เบา/กลาง/หนัก) เพื่อกำหนด METs แต่ตรวจสอบแล้วพบว่าฟีเจอร์นี้ไม่เคยทำงานจริง — แอปมือถือไม่เคยส่งค่าที่เลือกขึ้น API เลย เปลี่ยนมาให้ระบบอนุมาน METs อัตโนมัติจาก 3 อย่างที่ระบบรู้อยู่แล้วแทน โดยไม่ต้องพึ่งสายรัดวัดหัวใจ: (1) MET พื้นฐานของท่านั้นๆ ที่แอดมินตั้งไว้ (2) %1RM ของเซสชันเทียบกับสถิติที่ดีที่สุดของสมาชิกคนนั้นในท่านั้น และ (3) ความหนาแน่นของการฝึก (เวลารวม/จำนวนเซต) — ได้เป็น `Final MET = Session Base MET × k_load × k_density` แล้วเข้าสูตร net METs เดิมด้านบนตรงๆ
