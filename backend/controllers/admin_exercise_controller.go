...
// [OUTPUT] {data: []WeightExercise (พร้อม muscle_details หลัก+รอง), total, page, page_size}
func GetAdminWeightExercises(c *gin.Context) {
	...
	if err := query.Preload("MuscleDetails.MuscleGroup").Order("wet_difficulty ASC, wet_id ASC").Offset(offset).Limit(pageSize).Find(&exercises).Error; err != nil {
