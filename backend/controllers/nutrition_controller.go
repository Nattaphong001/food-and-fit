...
func CreateDailyNutrition(c *gin.Context) {
	...
	// dntt_time เป็น NOT NULL แล้ว (migrations/2026-09-06_align_defaults_and_nullability.sql)
	// client ที่ไม่ส่งเวลามาต้องได้เวลาปัจจุบันของเซิร์ฟเวอร์แทน ไม่ใช่ '' ซึ่ง MySQL ปฏิเสธว่า
	// ไม่ใช่ค่า TIME ที่ถูกต้อง
	if req.DnttTime == "" {
		req.DnttTime = time.Now().Format("15:04:05")
	}

	// อาหารที่ผู้ใช้กรอกเอง (ไม่ได้เลือกจากฐานข้อมูล) ต้องมีชื่ออาหารเสมอ — กรณีเลือกจากระบบไม่ต้อง
	// เช็คตรงนี้เพราะโค้ดด้านล่างเติมชื่อจากตาราง nutrition ให้อยู่แล้ว
	if req.NttID == nil && strings.TrimSpace(req.DnttFoodName) == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "กรุณาระบุชื่ออาหาร"})
		return
	}
