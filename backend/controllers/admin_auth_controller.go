...
func AdminLogin(c *gin.Context) {
	...
	// ยังเป็น NULL อยู่เท่านั้น login ครั้งต่อๆ ไปจะไม่ทับค่าเดิม
	if sysUser.SysStartDate == nil {
		now := time.Now()
		if err := config.DB.Model(&sysUser).Update("sys_start_date", now).Error; err != nil {
			log.Printf("AdminLogin: set sys_start_date failed for sys_id=%d: %v", sysUser.SysID, err)
		} else {
			sysUser.SysStartDate = &now
		}
	}
