	...
	api := r.Group("/api")
	{
		...
		admin := api.Group("/admin")
		{
			...
			adminProtected.Use(middleware.AdminAuthMiddleware())
			{
				...
				// 🛡️ จัดการผู้ดูแลระบบ (multi-admin) — list / create
		...
		protected.Use(middleware.AuthMiddleware())
		{
			...
			// --- ผลการฝึกเวทเทรนนิ่ง ---
			// POST   /workout-results        → บันทึกผลทั้งเซสชัน (ท่าเดียว หลายเซต + เวลารวม)
			//                                   Smart Auto Calorie (2026-09-08) — เปลี่ยนจากเดิมที่
			//                                   ยิงทีละเซต เพราะสูตรคำนวณ MET ระดับเซสชัน ต้องรู้
			//                                   ทุกเซตพร้อมกัน — ดู controllers.WeightSessionRequest
		...
		memberWorkout := protected.Group("/")
		{
			...
			// "แผนของฉัน" — แผนส่วนตัวของสมาชิกเอง แก้/ลบเองได้ ต่างจาก workout_plans/* ด้านบนที่เป็น
			// master data ร่วมกันทุกคน
			// ดีไซน์ 1:1: 1 สมาชิกมีแผน active ได้แค่ 1 แผนเสมอ รวมทุกประเภท (ระบบ+ส่วนตัว)
			// (2026-09-04 รอบ 2: ยุบตาราง member_workout_plans เข้า workout_schedules แล้ว — ไม่มี
			// ตารางแยกเก็บ header ของแผนอีกต่อไป ข้อมูลระดับแผน (ชื่อ/จำนวนวัน/แม่แบบต้นทาง) เก็บซ้ำ
			// อยู่ในทุกแถวของ workout_schedules ที่ mb_id เดียวกัน — ดู models/exercise.go)
			// (ตัด GetMyWorkoutPlans ออกแล้ว — เคย list หลายแผนพร้อมกัน ไม่มีจุดไหนเรียกอีกต่อไปหลังลบ
			// my_workout_plans_view.dart ฝั่ง Flutter ดู GetMemberActivePlan สำหรับแผน active แผนเดียว)
			// POST   /workout-plans/mine              → สร้างแผนส่วนตัวใหม่ (ลบแผนเดิม+ตารางฝึกเดิมทิ้งก่อนเสมอ)
			...
			// POST /workout-plans/fork {wpt_id} → คัดลอกแผนระบบเป็นแผนส่วนตัวใหม่แก้ไขได้อิสระ
			// ไม่มีปุ่มเรียกจาก UI แล้ว (เก็บไว้เผื่อทำฟีเจอร์ "สร้างแผนจากแม่แบบ" ในอนาคต) — ล้างแผน
			// เดิมทิ้งก่อน copy เสมอ (1 สมาชิกมีแผนได้แค่ 1 แผนเสมอ)
