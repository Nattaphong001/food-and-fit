...
class Exercise {
  ...
  // กลุ่มกล้ามเนื้อ "หลัก" ตัวแรก (exm_type=1) — ใช้จัดหมวดหมู่/ส่วนหัวที่ต้องมีกลุ่มเดียวต่อท่า
  final int? muscleGroupId;
  // ทุกกลุ่มกล้ามเนื้อที่ท่านี้เกี่ยวข้อง (หลัก+รอง จาก exercise_muscle_details) — ใช้ "กรอง" เท่านั้น
  // ห้ามใช้ muscleGroupId ตัวเดียวกรอง เพราะ weight_exercises.mug_id ถูก DROP ออกจาก DB ไปแล้ว
  // (2026-09-04) ข้อมูลจริง 71% ของท่ามีกล้ามเนื้อหลักมากกว่า 1 มัด (ดู admin_exercise_controller.go
  // บรรทัด ~29-34 ที่ยืนยันกับผู้ใช้แล้วให้ยึด exercise_muscle_details เป็นหลักเหมือน UX เดิม)
  ...
  factory Exercise.fromJson(Map<String, dynamic> json) {
    // API ปัจจุบัน (GetWeightExercises) คืน muscle_details: [{exm_type, mug_id, muscle_group:{mug_id,
    // mug_name,...}}, ...] แทน mug_id/muscle_group เดี่ยวๆ แบบเดิม (คอลัมน์ weight_exercises.mug_id
    // ถูก DROP ไปแล้ว) — ต้องอ่านผ่าน list นี้เท่านั้น
    final details = (json['muscle_details'] as List?) ?? const [];
    final groupIds = <int>[];
    Map<String, dynamic>? primary;
    int? primaryId;
    for (final raw in details) {
      final d = raw as Map<String, dynamic>;
      final mugId = (d['mug_id'] as num?)?.toInt();
      if (mugId != null && !groupIds.contains(mugId)) groupIds.add(mugId);
      if (d['exm_type'] == 1 && primary == null) {
        primary = d['muscle_group'] as Map<String, dynamic>?;
        primaryId = mugId;
      }
    }
    primaryId ??= groupIds.isNotEmpty ? groupIds.first : null;
    return Exercise(
