...
class _ProfileEditBodyViewState extends State<ProfileEditBodyView> {
  ...
  // --------------------------------------------
  // [FEATURE] BMR_TDEE
  // [FUNCTION] _saveBodyStats
  // [DESCRIPTION] validate ส่วนสูง/น้ำหนักฝั่ง client แล้วยิง PUT /member/body-stats ครั้งเดียว
  //               ส่ง gender/birth_date รวมไปในคำขอเดียวกัน (เดิมยิง updateProfile() แยกก่อน
  //               updateBodyStats() ทำให้ backend สร้างแถวประวัติ BMI/BMR/TDEE ซ้ำซ้อนต่อการ
  //               บันทึก 1 ครั้ง — ตัดออกแล้ว) ถ้า backend ส่ง warning กลับมาว่าค่าที่เปลี่ยนดู
  //               ผิดปกติ จะเด้ง dialog ให้ผู้ใช้ยืนยันซ้ำก่อนปิดหน้า (ไม่ block การบันทึก
  //               backend บันทึกข้อมูลไปแล้วตั้งแต่ก่อนโชว์ dialog นี้)
  // [INPUT] ค่าจาก _heightController/_weightController (ซม./กก.), _gender, _birthDate,
  //         _activityLevel, _fitnessGoal (state ของหน้านี้)
  // [OUTPUT] pop หน้านี้กลับพร้อม {'stats': BodyStats, 'target': int} เมื่อบันทึกสำเร็จ
  // [TABLES] member_profile, member_body_stats, member_bmr_history
  // [RELATED] PROFILE
  // --------------------------------------------
  Future<void> _saveBodyStats() async {
    ...
    if (warnings.isNotEmpty) {
      final proceed = await _showPlausibilityWarningDialog(warnings);
      if (!mounted || !proceed) return; // เลือก "กลับไปแก้ไข" — อยู่หน้านี้ต่อ ไม่ปิดหน้า
    }
  ...
  // --------------------------------------------
  // [FEATURE] PROFILE
  // [FUNCTION] _showPlausibilityWarningDialog
  // [DESCRIPTION] แสดง dialog แจ้ง warning ที่ backend ส่งกลับมาพร้อมผลบันทึกสำเร็จ (น้ำหนัก/
  //               ส่วนสูงเปลี่ยนจากแถวก่อนหน้ามากผิดปกติ) ไม่ block การบันทึก — backend บันทึก
  //               ข้อมูลไปแล้วก่อนหน้านี้ แค่ให้ผู้ใช้ยืนยันซ้ำว่าค่าที่กรอกถูกต้องจริง หรือกลับไป
  //               แก้ไขอีกครั้ง (แก้แล้วกดบันทึกซ้ำจะทับแถวของวันนี้ ไม่สร้างแถวซ้อนใหม่)
  // [INPUT] warnings: ข้อความ warning จาก backend (List<String>)
