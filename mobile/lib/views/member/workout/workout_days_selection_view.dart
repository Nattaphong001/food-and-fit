...
class WorkoutDaysSelectionView extends StatefulWidget {
  ...
  @override
  State<WorkoutDaysSelectionView> createState() =>
      _WorkoutDaysSelectionViewState();
}
...
class _WorkoutDaysSelectionViewState extends State<WorkoutDaysSelectionView> {
  ...
  Future<void> _fetchPlanTemplates() async {
    ...
    try {
      final List<dynamic> templates = await WorkoutService.to
          .getPlanTemplates();
  ...
  // ดีไซน์ใหม่ (2026-09-04): user มีแผน active ได้แค่ 1 แผนเสมอ เลือกแผนนี้แล้วแผนที่ใช้อยู่ตอนนี้
  // (ระบบตัวอื่น หรือแผนส่วนตัว) จะถูกลบถาวรจาก workout_schedules — ข้าม กรณีเลือกแผนระบบตัวเดิมที่
  // ใช้อยู่แล้วซ้ำ เพราะไม่มีอะไรถูกลบจริง (SelectWorkoutPlan ฝั่ง backend no-op เคสนี้เหมือนกัน)
  // คำเตือนนี้เคยเป็น dialog แยกอีกชั้นหลังกด "ใช้แผนนี้" — รวมเป็น dialog เดียวแทน (เดิมกดยืนยัน
  // 2 รอบซ้อนกันสำหรับการกระทำเดียว) แสดงเป็นแถบแดงในเนื้อหาเดียวกัน + ปุ่มยืนยันเปลี่ยนสี/ข้อความ
  // ตามว่ากำลังจะลบแผนเดิมทิ้งจริงหรือเปล่า
  Future<void> _confirmAndSelectPlan(
    ...
    final isSameSystemPlanAlreadyActive =
        svc.activePlanType != 'custom' &&
        svc.activePlanType != 'copied' &&
        svc.activePlanId == wptId;
    final willReplaceActivePlan =
        svc.activePlanId != null && !isSameSystemPlanAlreadyActive;
    final isCurrentCustom =
        svc.activePlanType == 'custom' || svc.activePlanType == 'copied';
      ...
      builder: (ctx) => Dialog(
        ...
        child: Column(
          ...
          children: [
                      ...
                      child: Row(
                        ...
                        children: [
                          ...
                          Expanded(
                            child: Text(
                              isCurrentCustom
                                  ? 'ท่าฝึกและตารางของแผนส่วนตัว "${svc.activePlanName}" จะถูกลบถาวรทันที'
                                  : 'ท่าฝึกและตารางของแผน "${svc.activePlanName}" ที่ใช้อยู่ตอนนี้จะถูกลบถาวรทันที',
                              style: TextStyle(
                                fontSize: 12,
            ...
            // Buttons — ยกเลิก=เทาเรียบ (ทางปลอดภัย), ยืนยัน=เขียวปกติ/แดงหนา (ถ้าจะลบแผนเดิมทิ้ง)
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  // ปุ่มยืนยันแบบทำลายข้อมูล (willReplaceActivePlan=true) เป็นปุ่มกรอบแดง — กรอบ/สไตล์
                  // เดียวกับ _confirmReset ใน weight_training_schedule_view.dart และ dialog ตั้งชื่อแผน
                  // ใน workout_view.dart (เดิมที่นี่เป็นปุ่มพื้นแดงทึบ ไม่ตรงกับอีก 2 จุด)
                  if (willReplaceActivePlan)
                    Expanded(
                      child: OutlinedButton(
                      ...
                      SliverPadding(
                        ...
                        sliver: SliverList(
                          delegate: SliverChildBuilderDelegate((
                            ...
                            final int days =
                                plan['wpt_days_per_week'] ??
                                plan['plan_days_per_week'] ??
                                3;
                            ...
                            return _buildWorkoutSection(
                              ...
                              description:
                                  plan['wpt_description'] ??
                                  plan['plan_description'] ??
                                  'บริหารร่างกายตามตารางที่จัดไว้ให้',
                              hasBorder: true,
