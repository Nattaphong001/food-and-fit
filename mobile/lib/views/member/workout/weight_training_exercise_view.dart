...
class _WeightTrainingExerciseViewState
  ...
  // ตรวจสอบย้อนหลังเท่านั้น สูตร Smart Auto Calorie (2026-09-08) ไม่ได้ใช้ค่านี้คำนวณแคลอรี่
  ...
  Future<void> _saveWorkoutToApi() async {
    ...
    // ท่านี้มาจากตารางฝึกอยู่แล้วเสมอ (widget.wschId ติดมาจาก schedule view) — ใช้ตรงๆ
    ...
    // ป็อปกลับเฉพาะตอนสำเร็จ — ถ้าไม่สำเร็จให้ user เห็น alert ค้างอยู่ก่อน แล้วเลือกเองว่าจะ
    // ลองใหม่หรือออก ไม่ปิดหน้าจอปิดบังว่าข้อมูลทั้งเซสชันไม่ถูกบันทึกเลย
    if (result['success'] == true) {
    ...
    return PopScope(
      ...
      body: Stack(
        children: [
          ...
          // 2. ปุ่มย้อนกลับ (ด้านบนซ้าย) — ขณะพักกดไม่ได้ (IgnorePointer กัน fallback
          // Navigator.pop ใน AppBackButton เองถ้าส่ง onTap เป็น null ตรงๆ) และจางหายไป
          // พร้อมพื้นหลังมืดลงด้วย opacity ผูกกับ _sheetT ตัวเดียวกับนาฬิกาหลักที่ footer
          // (ดู _buildActiveRow) ให้ทั้งคู่ "drop" ไปพร้อมกันตอน sheet ยืดขึ้นระหว่างพัก
          Positioned(
            left: 20,
