...
class WorkoutService extends GetxService {
  ...
  // ชื่อเล่นแผนระบบที่ผู้ใช้ตั้งเอง — เก็บฝั่ง client เท่านั้น ไม่ยิงขึ้น server เลย เพราะ wpt_name
  // เป็นชื่อกลางของแผนระบบใน workout_plan_template ที่ทุกคนเห็นร่วมกัน แก้ตรงนั้นจะกระทบผู้ใช้คนอื่น
  // ที่ใช้แผนเดียวกันด้วย — ตั้งชื่อเล่นเฉพาะตัวแทน คีย์ตาม wpt_id กันชนกันข้ามแผน สลับไปใช้แผนระบบ
  // อื่นจะไม่เห็นชื่อเล่นแผนเก่าเพราะคีย์คนละอัน ไม่ต้องเคลียร์เอง
  String? getSystemPlanNickname(int wptId) => _storage.read<String>('system_plan_nickname_$wptId');
  void setSystemPlanNickname(int wptId, String name) => _storage.write('system_plan_nickname_$wptId', name);
  ...
  // ==========================================
  // 3a. "แผนของฉัน" — ดีไซน์ 1:1 (2026-09-04): สมาชิกมีแผนส่วนตัวได้สูงสุด 1 แผนเสมอ และมีแผน
  // active ได้แค่ 1 แผนรวมทุกประเภท (ดู GetMemberActivePlan ด้านล่างสำหรับแผน active แผนเดียว)
  // ==========================================
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (body['has_plan'] == true) {
          ...
          final serverName = (d['plan_name'] as String?) ?? '$days วัน/สัปดาห์';
          final isCustom  = d['is_custom'] as bool? ?? false;
          // แผนระบบ: ถ้าผู้ใช้เคยตั้งชื่อเล่นไว้ ให้ใช้ชื่อเล่นแทนชื่อกลางจาก server เสมอ (ฟังก์ชันนี้
          // ถูกเรียกซ้ำทุกครั้งที่กลับมาหน้าแผน กันไม่ให้ชื่อเล่นโดนทับด้วยชื่อจริงจาก server)
          final name = isCustom ? serverName : (getSystemPlanNickname(planId) ?? serverName);
  ...
  // ดึงสถานะแผน active เดียวของสมาชิก (has_plan/is_custom/is_modified/days_per_week)
  // จาก GetMemberActivePlan — ใช้แทน getMyPlans() เดิมที่ backend ตัดออกแล้ว (2026-09-04 รอบ 2
  // ยุบ member_workout_plans เข้า workout_schedules สมาชิกมีแผนได้แค่ 1 แผนเสมอ ไม่มี list ให้ดึงอีก)
  Future<Map<String, dynamic>> getActivePlanStatus() async {
    try {
      final response = await _api.get('/member/workout-plans/active');
      if (response.statusCode == 200) {
    ...
    try {
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        final estimated1rm = (response.data is Map)
            ...
            : 0.0;
        final caloriesBurned = (response.data is Map)
            ? (response.data['calories_burned'] as num?)?.toDouble()
            : null;
        return {'success': true, 'estimated_1rm': estimated1rm, 'calories_burned': caloriesBurned};
      }
