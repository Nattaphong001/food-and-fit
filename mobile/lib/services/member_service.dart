...
class MemberService extends GetxService {
  ...
  // [DESCRIPTION] เรียก PUT /member/body-stats — ส่งน้ำหนัก/ส่วนสูง/กิจกรรม/เป้าหมาย (+ gender/
  //               birth_date แบบ optional รวมในคำขอเดียว) backend คำนวณ BMI/BMR/TDEE/Target
  //               ใหม่แล้วส่งกลับมา พร้อม warnings ถ้าค่าที่เปลี่ยนดูผิดปกติ (ไม่ block การบันทึก)
  // [INPUT] data: Map ต้องมี height(ซม.)/weight(กก.)/activity_level/target, มี gender/birth_date
  //         ได้ (optional)
  // [OUTPUT] Map — success, message, data (BodyStats จาก backend), warnings (List<String>)
  ...
  Future<Map<String, dynamic>> updateBodyStats(Map<String, dynamic> data) async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final warnings = statsData is Map<String, dynamic>
            ? (statsData['warnings'] as List?)?.map((e) => e.toString()).toList() ?? const <String>[]
            : const <String>[];
        return {
          'success': true,
          'message': raw['message'] ?? 'อัพเดทข้อมูลร่างกายสำเร็จ',
          'data': stats,
          'warnings': warnings,
        };
      }
